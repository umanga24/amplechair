            ...
            <table class="table table-striped table-bordered table-hover" id="example-table" cellspacing="0" width="100%">
                ...
                <tbody>
                    ...
                    <tr class="category_row{{$slider_data->id}}">
                        ...
                        <td class="changeStatus{{$slider_data->id}}">

                            <span class="btn btn-rounded btn-sm {{($slider_data->status == 'Publish') ? 'btn-success':   'btn-warning'}}  ChangeSliderStatus" data-status="{{$slider_data->status}}" data-data_id = "{{$slider_data->id}}" style="cursor: pointer;">{{$slider_data->status}}ed</span>
            ...
            $.ajax({
                ...
                success : function(response){
                    ...
                    if (response.status == true) {
                        ...
                        var replace_html = '<span class="btn btn-rounded btn-sm btn-'+((update_data.status == 'Publish')? 'success' : 'warning')+' ChangeSliderStatus" data-status="'+update_data.status+'" data-data_id = "'+update_data.id+'" style="cursor: pointer;" data-toggle="tooltip" data-placement="top" title= "Make '+((update_data.status == 'Publish')? 'Unpublish' : 'Publish')+' this Slider.">'+update_data.status+'ed</span>';
